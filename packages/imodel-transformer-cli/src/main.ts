...
import { runTransformation } from "./transformer.js";
...
async function main(): Promise<void> {
  const argv = await yargs(hideBin(process.argv))
    .scriptName("imodel-transformer-cli")
    .usage("$0 --source <file> --output <file>")
    .option("source", {
      alias: "s",
      type: "string",
      demandOption: true,
      describe: "Path to the existing iModel file to transform",
    })
    .option("output", {
      alias: "o",
      type: "string",
  ...
  const result = await runTransformation({ sourcePath, outputPath });
